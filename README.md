...
```bash
npm run dev -- "<会社名>" "<会社URL>" "<ドメイン>" "<部署>" [--debug]
```

例

```bash
npm run dev -- "株式会社さくらケーシーエス" "https://www.kcs.co.jp/ja/index.html" "kcs.co.jp" "情報システム部"
```

- 引数:
  - `<会社名>`: 例 `株式会社さくらケーシーエス`
  - `<会社URL>`: 例 `https://www.kcs.co.jp/ja/index.html`
  - `<ドメイン>`: 例 `kcs.co.jp`
  - `<部署>`: 例 `情報システム部`
- オプション:
...
npm start -- "<会社名>" "<会社URL>" "<ドメイン>" "<部署>" [--debug]
```
